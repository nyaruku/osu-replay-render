...
GuiWindowFileDialogState InitGuiWindowFileDialog(const char *initPath)
{
    ...
    // Init window data
    state.windowBounds = (Rectangle){ GetScreenWidth()/2 - 440/2, GetScreenHeight()/2 - 310/2, 800, 400 };
    ...
    if (state->windowActive)
    {
        ...
        GuiSetStyle(LISTVIEW, LIST_ITEMS_HEIGHT, 24);
            ...
            GuiListViewEx((Rectangle){ state->windowBounds.x + 8, state->windowBounds.y + 48 + 20, state->windowBounds.width - 16, state->windowBounds.height - 60 - 16 - 68 },
                dirFilesIcon, state->dirFiles.count, &state->filesListScrollIndex, &state->filesListActive, &state->itemFocused);
        #endif
        ...
        //--------------------------------------------------------------------------------------
        GuiLabel((Rectangle){ state->windowBounds.x + 8, state->windowBounds.y + state->windowBounds.height - 68, 90, 24 }, "File name:");
        if (GuiTextBox((Rectangle){ state->windowBounds.x + 102, state->windowBounds.y + state->windowBounds.height - 68, state->windowBounds.width - 214, 24 }, state->fileNameText, 128, state->fileNameEditMode))
        {
        ...
        GuiLabel((Rectangle){ state->windowBounds.x + 8, state->windowBounds.y + state->windowBounds.height - 24 - 12, 98, 24 }, "File filter:");
        GuiComboBox((Rectangle){ state->windowBounds.x + 102, state->windowBounds.y + state->windowBounds.height - 24 - 12, state->windowBounds.width - 214, 24 }, (state->filterExt[0] != '\0') ? state->filterExt : "All files", &state->fileTypeActive);
...
static void ReloadDirectoryFiles(GuiWindowFileDialogState *state)
{
    ...
    state->dirFiles = LoadDirectoryFilesEx(state->dirPathText, NULL, false);

    if (state->filterExt[0] != '\0') {
        unsigned int kept = 0;
        for (unsigned int i = 0; i < state->dirFiles.count; i++) {
            bool isDir = DirectoryExists(state->dirFiles.paths[i]);
            bool matches = IsFileExtension(state->dirFiles.paths[i], state->filterExt);
            if (isDir || matches) {
                // SWAP so each allocation stays at exactly one position
                // (copy would leave duplicate pointers -> double free on UnloadDirectoryFiles)
                char *tmp = state->dirFiles.paths[kept];
                state->dirFiles.paths[kept] = state->dirFiles.paths[i];
                state->dirFiles.paths[i] = tmp;
                kept++;
            }
        }
        state->dirFiles.count = kept;
    }

    if (state->dirFiles.count > MAX_DIRECTORY_FILES)
        state->dirFiles.count = MAX_DIRECTORY_FILES;
