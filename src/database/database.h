...
namespace Database {
    ...
    std::optional<BeatmapEntry> findByMd5(BeatmapDb& beatmapDb, const std::string& md5);

    namespace Helper {

        inline std::string columnText(sqlite3_stmt* stmt, int column) {
        ...
        inline bool upToDate(sqlite3* db, const std::filesystem::path& path, const int64_t lastModification) {
            sqlite3_stmt* stmt;
            ...
            if (sqlite3_step(stmt) == SQLITE_ROW) {
                result = sqlite3_column_int64(stmt, 0) == lastModification;
            }
        ...
        inline std::unordered_map<std::string, int64_t> loadAllModTimes(sqlite3* db) {
            ...
            while (sqlite3_step(stmt) == SQLITE_ROW) {
                result[columnText(stmt, 0)] = sqlite3_column_int64(stmt, 1);
            }
            sqlite3_finalize(stmt);
            return result;
        }

        inline void upsert(sqlite3* db, const BeatmapEntry& entry, int64_t lastModification) {
            sqlite3_stmt* stmt;
            ...
            sqlite3_bind_int64(stmt, 3, lastModification);
            ...
            sqlite3_bind_int(stmt, 8, entry.beatmapId);
            sqlite3_bind_int(stmt, 9, entry.beatmapSetId);
            sqlite3_step(stmt);
            sqlite3_finalize(stmt);
        }
    }
    ...
    inline std::optional<BeatmapEntry> findByMd5(BeatmapDb& beatmapDb, const std::string& md5) {
        const auto sql =
            "SELECT file_path, md5, title, artist, creator, version, beatmap_id, beatmap_set_id "
            "FROM beatmaps WHERE md5 = ? LIMIT 1;";
        sqlite3_stmt* stmt;
        if (sqlite3_prepare_v2(beatmapDb.db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
            return std::nullopt;
        }
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            BeatmapEntry e;
            e.filePath = Helper::columnText(stmt, 0);
            e.md5 = Helper::columnText(stmt, 1);
            e.title = Helper::columnText(stmt, 2);
            e.artist = Helper::columnText(stmt, 3);
            e.creator = Helper::columnText(stmt, 4);
            e.version = Helper::columnText(stmt, 5);
            e.beatmapId = sqlite3_column_int(stmt, 6);
            e.beatmapSetId = sqlite3_column_int(stmt, 7);
            result = e;
        }
        sqlite3_finalize(stmt);
        return result;
